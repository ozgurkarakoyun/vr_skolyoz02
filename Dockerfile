...
# 5. Ultralytics (marker tespiti için)
RUN pip install --no-cache-dir ultralytics==8.3.40 --no-deps

# 6. KRİTİK: Tüm opencv'leri kaldır, sadece headless kal
...
CMD ["sh", "-c", "gunicorn --worker-class eventlet -w 1 --bind 0.0.0.0:$PORT --timeout 180 app:app"]
